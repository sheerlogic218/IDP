...
class MainMotors {
    ...
    void change_ML_speed(int delta) {
        cout << "change ML speed";
        // l_speed += delta;
        // set_ML_speed(l_speed);
    }
    void change_MR_speed(int delta) {
        cout << "change MR speed";
        // r_speed += delta;
        // set_MR_speed(r_speed);
    }
...
class Servo_claws {
  ...
  void open(){
    cout << "claws open"; //steady_turn(open_angle);
  }
  void close(){
    cout << "claws closed"; //go_zero();
  }
  void straight_ahead(){
    cout << "claws straight ahead"; //steady_turn(30);
  }
};
Servo_claws Claws;
...
void read_sensors(){
    ...
    fls_state = 0;//digitalRead(far_left_sensor);
    ls_state = 1;//digitalRead(left_sensor);
    rs_state = 1;//digitalRead(right_sensor);
    frs_state = 0;//digitalRead(far_right_sensor);
}
